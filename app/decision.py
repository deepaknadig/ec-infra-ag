...
def fertilizer_decision(N, P, K, NDVI, wind_speed):
	...
	if wind_speed>max_wind_speed+2:
		...
		if N_val<avg_N or P_val<avg_P or K_val<avg_K: 
			if NDVI<0.3 and NDVI>=0.2: 
				decision = Decision.SPRAY_MORE
			elif NDVI<0.51 and NDVI>=0.3: 
				decision = Decision.SPRAY_LESS
		else:
			decision = Decision.NO_SPRAYING
	return json.dumps({"decision":decision.name})
# print(fertilizer_decision(N, P, K, NDVI, wind_speed))
